...
def inject_glitch_css(l, t, w, h):
    ...
    <style>
        ...
        div[data-testid="stImage"]::before {{
            ...
            background-image: url({g_url}); background-size: cover;
            mix-blend-mode: hard-light; opacity: 0.8;
            border: 1px solid rgba(0, 255, 0, 0.5);
            animation: glitch-flicker 0.1s infinite;
        }}
        @keyframes glitch-flicker {{
            0% {{ opacity: 0.8; filter: hue-rotate(0deg); }}
            50% {{ opacity: 0.6; filter: hue-rotate(180deg) invert(0.2); }}
...
# --- RANDOMIZERS ---
def move_glitch():
    # Totally random position every time
