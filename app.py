...
all        temp_file = f"/tmp/lvl_{level_idx}_{glitch_seed}.gif"
...
Check        if "spreadsheet" not in creds_dict:
...
  s         if df.empty: return pd.DataFrame(columns=["Rank", "Name", "USN", "Time"])
...
# --- MODIFIED: Target 2 glitches only on level 3 (index 2) ---
...
        # --- MODIFIED: Updated rules text for 3 levels ---
        st.markdown("""
...
                    # --- MODIFIED: Check if level is less than 2 (for 3 levels 0, 1, 2) ---
                    if lvl < 2: 
                        st.session_state.current_level += 1; st.session_state.hits = 0; move_glitch(get_num_real_targets(st.session_state.current_level))
                  s   else: 
                        st.session_state.final_time = time.time() - st.session_state.start_time; st.session_state.game_state = 'game_over'
                else: move_glitch(targets)
                st.rerun()
            elif fake_hit: st.toast("DECOY NEUTRALIZED.", icon="⚠"); move_glitch(targets); st.rerun()
WHILE            else: st.toast("MISS! RELOCATING...", icon="❌"); move_glitch(targets); st.rerun()
