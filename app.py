...
# --- GOOGLE SHEETS SETUP --- 
# IMPORTANT: Make sure the following are done outside this code:
# 1) Enable Google Sheets and Drive APIs in Google Cloud
# 2) Create a Service Account with credentials JSON downloaded
# 3) Share your Google Sheet with the service account email
# 4) Copy your service account JSON credentials content into your .streamlit/secrets.toml as described below

# EXAMPLE: Your .streamlit/secrets.toml should contain something like:
#
# [connections.gsheets]
...
# client_x509_cert_url = "url_from_service_account_json"
...
try:
    ...
    st.error(f"Failed to establish Google Sheets connection: {e}")
...
def get_leaderboard():
    if conn:
        try:
            ...
            if df.empty or not {'Tag', 'Name', 'USN', 'Time'}.issubset(df.columns):
                st.info("Leaderboard data missing or incomplete. Please check 'Scores' sheet headers (Tag, Name, USN, Time) and data.")
                return pd.DataFrame(columns=["Rank", "Name", "USN", "Time"])
            df = df.dropna(subset=['Tag', 'Name', 'USN', 'Time']).copy()
            ...
            st.error(f"Error reading leaderboard: {e}")
            return pd.DataFrame(columns=["Rank", "Name", "USN", "Time"])
    else:
        st.info("No connection to Google Sheets. Check your secrets configuration and permissions.")
        return pd.DataFrame(columns=["Rank", "Name", "USN", "Time"])
...
if st.session_state.game_state == "menu":
    ...
    if conn and not leaderboard_df.empty:
        ...
        st.info("Ensure the 'Scores' sheet has headers: Tag, Name, USN, Time and contains at least one score row.")
    else:
        st.error("❌ Leaderboard Connection Failed. Check secrets and sharing permissions.")
        st.dataframe(leaderboard_df, hide_index=True, use_container_width=True) 
