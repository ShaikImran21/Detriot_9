...
if 'game_state' not in st.session_state:
    st.session_state.update({'game_state': 'menu', 'current_level': 0, 'start_time': 0.0, 'player_tag': 'UNK', 'final_time': 0.0, 'last_move_time': time.time(), 'glitch_seed': random.randint(1, 100000), 'current_box': get_new_glitch_box()})
...
def move_glitch():
    ...
    st.session_state.glitch_seed = random.randint(1, 100000) # New seed for new GIF
...
# AUTO-TELEPORT LOGIC (Runs once per interaction)
...
if st.session_state.game_state == "menu":
    ...
    # Generate and display the new GIF
    ...
    if gif_path and scaled_box:
        ...
        if coords:
            # Check if they were too slow (clicked after timer expired)
            if time.time() - st.session_state.last_move_time > MOVE_DELAY:
