...
GAME_WIDTH = 700
HIT_TOLERANCE = 50
MOVE_DELAY = 4.0  # 4 seconds to find it before it moves
...
# --- THE EXACT QUANTUM GLITCH ANIMATION YOU REQUESTED ---
...
def generate_scaled_gif(img_path, original_box, target_width, level_idx, glitch_seed):
    try:
        ...
        frames = []
        # 15 frames normal (~3s wait)
        for _ in range(15): frames.append(base_img.copy())
        # 8 frames chaotic mutation (~0.8s visible)
...
if st.session_state.game_state == "menu":
    ...
    if gif_path and scaled_box:
        # Key includes seed so it reloads when it moves
